...
class ReporteController extends Controller
{
    ...
    public function reporteEntregas(Request $request): JsonResponse
    {
        ...
        // Resumen general
        $resumen = [
            'total_repartos' => Reparto::whereBetween('fecha_programada', [$desde, $hasta])->count(),
            'total_entregados' => Reparto::whereBetween('fecha_programada', [$desde, $hasta])
                                        ->where('estado', 'entregado')->count(),
            'total_fallidos' => Reparto::whereBetween('fecha_programada', [$desde, $hasta])
                                      ->where('estado', 'fallido')->count(),
            'km_totales' => Reparto::whereBetween('fecha_programada', [$desde, $hasta])
                                  ->sum('km_recorridos') ?? 0,
